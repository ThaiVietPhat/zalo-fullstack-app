...
Spring Boot 3.4.2, Java 21, Maven. Production DB: Railway MySQL addon (cùng internal network). File storage: Cloudflare R2 (S3-compatible, endpoint override qua `AWS_S3_ENDPOINT`).
...
├── call/           — WebRTC signaling relay + call history (CallSession entity)
│   ├── controller/ — CallSignalingController (@MessageMapping), CallHistoryController (REST)
│   ├── dto/        — CallSignalDto, CallSessionDto, StartCallRequest
│   ├── entity/     — CallSession (CallType: VOICE/VIDEO, CallStatus: MISSED/ENDED/REJECTED)
│   ├── repository/ — CallSessionRepository (findByChatIdOrderByStartedAtDesc)
...
- Railway MySQL là MySQL chuẩn — **có thể dùng `IF NOT EXISTS`** trong `ALTER TABLE ADD COLUMN`        
...
- `/app/call/signal` — client gửi WebRTC signal (backend relay theo `targetUserId`)
...
- `/topic/call/{userId}` — WebRTC signal relay đến target user (call-offer/answer/reject/cancel/end/ice-candidate)
...
1-1 message: backend gửi **cả hai**: `sendChatBroadcast()` → `/topic/chat/{id}` VÀ `sendMessageNotification()` → `/user/{email}/queue/messages`. Frontend/Mobile xử lý cả hai nhưng chỉ `/topic` mới đáng tin cho UI update.
...
POST   /api/v1/calls              — lưu call session (chỉ caller gửi, tránh duplicate)
GET    /api/v1/calls/{chatId}     — lấy lịch sử cuộc gọi theo chatId
```