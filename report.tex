...
\raggedbottom                  % Tránh giãn dòng gây trang trắng
...
\begin{figure}[H]
    ...
    \begin{plantuml}
        ...
        package "Hồ sơ và Kết bạn" {
            (Quản lý hồ sơ)
            (Tìm kiếm bạn bè)
            (Kết bạn)
        }

        package "Nhắn tin và Nhóm" {
            (Nhắn tin 1-1)
            (Gửi media)
            (Quản lý nhóm)
        }

        package "Tính năng mở rộng" {
            (AI trò chuyện)
            (Bài viết)
            (Cảm xúc bài viết)
        }
...
\subsection{Use case Khách (Guest)}
...
\begin{figure}[H]
    ...
    \begin{plantuml}
        ...
        G --> (Đăng ký tài khoản)
        G --> (Xác thực email)
        G --> (Gửi lại mã xác thực)
        G --> (Đăng nhập)
        G --> (Quên mật khẩu)
        G --> (Đặt lại mật khẩu)

        (Đăng ký tài khoản) ..> (Xác thực email) : <<include>>
        (Quên mật khẩu) ..> (Đặt lại mật khẩu) : <<include>>
        @enduml
    \end{plantuml}
    \caption{Use case dành cho khách (chưa đăng nhập)}
\end{figure}

\subsection{Use case Người dùng (User)}
...
\begin{figure}[H]
    ...
    \begin{plantuml}
        ...
        package "Nhắn tin 1-1" {
            (Bắt đầu/Xem danh sách chat)
        ...
        package "An toàn" {
            (Chặn/Mở khóa người dùng)
            (Báo cáo vi phạm)
        }

        U --> (Xem/Sửa hồ sơ)
        U --> (Đổi mật khẩu)
        U --> (Upload ảnh đại diện)
        U --> (Đăng xuất)
        U --> (Tìm kiếm người dùng)
        U --> (Gửi lời mời kết bạn)
        U --> (Chấp nhận/Từ chối)
        U --> (Xem danh sách bạn bè)
        U --> (Hủy kết bạn)
        U --> (Bắt đầu/Xem danh sách chat)
        U --> (Gửi tin nhắn văn bản)
        U --> (Gửi file/media)
        U --> (Thu hồi tin nhắn)
        U --> (Xóa tin nhắn)
        U --> (Đánh dấu đã xem/gửi)
        U --> (Tạo nhóm)
        U --> (Mời/Xóa thành viên)
        U --> (Rời nhóm/Giải tán)
        U --> (Phân quyền admin)
        U --> (Gửi tin nhắn nhóm)
        U --> (Ghim tin nhắn)
        U --> (Gọi thoại/video)
        U --> (Xem lịch sử cuộc gọi)
        U --> (Thả/Gỡ cảm xúc)
        U --> (Trò chuyện với AI)
        U --> (Gợi ý trả lời thông minh)
        U --> (Tóm tắt hội thoại)
        U --> (Chặn/Mở khóa người dùng)
        U --> (Báo cáo vi phạm)

        (Gửi tin nhắn văn bản) <.. (Gửi file/media) : <<extend>>
        (Gửi tin nhắn văn bản) <.. (Thu hồi tin nhắn) : <<extend>>
        (Gửi tin nhắn nhóm) <.. (Ghim tin nhắn) : <<extend>>
        @enduml
    \end{plantuml}
    \caption{Use case dành cho người dùng đã đăng nhập}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{plantuml}
        ...
        A --> (Xem chi tiết người dùng)
        A --> (Cấm/Bỏ cấm người dùng)
        A --> (Xóa người dùng)
        A --> (Phân/Thu hồi quyền Admin)
        A --> (Đặt lại mật khẩu)
        A --> (Xem danh sách nhóm)
        A --> (Xóa nhóm)
        A --> (Xem thống kê hệ thống)
        A --> (Xem nhật ký hệ thống)
        A --> (Xem danh sách báo cáo)
        A --> (Giải quyết báo cáo)
        A --> (Tạo tài khoản Admin)
        A --> (Hỗ trợ người dùng)
        @enduml
    \end{plantuml}
    \caption{Use case dành cho quản trị viên}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|c|X|}
        ...
        \hline
        Khách (Guest) & Người dùng chưa đăng nhập, có thể đăng ký tài khoản mới, đăng nhập hoặc khôi phục mật khẩu \\
        \hline
        Người dùng (User) & Thành viên đã đăng nhập, sử dụng các tính năng nhắn tin 1-1, nhóm, gọi thoại/video, kết bạn, quản lý hồ sơ, trò chuyện AI, chặn/báo cáo người dùng \\
        \hline
        Quản trị viên (Admin) & Người dùng có quyền quản trị, quản lý người dùng/nhóm, xử lý báo cáo, xem thống kê và nhật ký hệ thống, hỗ trợ người dùng \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{longtable}{|c|p{11cm}|}
    ...
    UC18 & Xác thực email \\
    \hline
    UC19 & Quên mật khẩu / Đặt lại mật khẩu \\
    \hline
    UC20 & Đổi mật khẩu \\
    \hline
    UC21 & Tải ảnh đại diện \\
    \hline
    UC22 & Đăng xuất \\
    \hline
    UC23 & Từ chối lời mời kết bạn \\
    \hline
    UC24 & Xem danh sách bạn bè \\
    \hline
    UC25 & Hủy kết bạn \\
    \hline
    UC26 & Thu hồi tin nhắn \\
    \hline
    UC27 & Xóa tin nhắn cá nhân \\
    \hline
    UC28 & Đánh dấu đã xem / đã gửi \\
    \hline
    UC29 & Cập nhật thông tin nhóm \\
    \hline
    UC30 & Rời nhóm / Giải tán nhóm \\
    \hline
    UC31 & Phân quyền admin nhóm \\
    \hline
    UC32 & Ghim / Bỏ ghim tin nhắn \\
    \hline
    UC33 & Mở khóa người dùng \\
    \hline
    UC34 & Xem lịch sử cuộc gọi \\
    \hline
    UC35 & Gỡ cảm xúc khỏi tin nhắn \\
    \hline
    UC36 & Gợi ý trả lời thông minh (AI) \\
    \hline
    UC37 & Tóm tắt hội thoại (AI) \\
    \hline
    UC38 & Xóa lịch sử AI \\
    \hline
    UC39 & Quản lý nhóm (Admin) \\
    \hline
    UC40 & Xem nhật ký hệ thống (Admin) \\
    \hline
    UC41 & Hỗ trợ người dùng (Admin) \\
    \hline
    UC42 & Tạo tài khoản admin \\
    \hline
...
\begin{figure}[H]
    ...
    \begin{plantuml}
        ...
        @enduml
    \end{plantuml}
    \caption{Class diagram --- Core chat}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{plantuml}
        ...
        @enduml
    \end{plantuml}
    \caption{Class diagram --- Group chat}
\end{figure}
