...
import { getSmartReplies } from '../../api/groupAi';

/**
 * SmartReplySuggestions — gợi ý 3 câu trả lời nhanh
 *
 * Props:
 *   groupId           — UUID nhóm
 ...
 */
export default function SmartReplySuggestions({ groupId, latestMessage, onSelect, inputValue }) {
  ...
  useEffect(() => {
    ...
    if (latestMessage.id === lastMessageIdRef.current) return; // đã fetch rồi
    if (!latestMessage.content) return; // tin nhắn media / bot / system → bỏ qua
